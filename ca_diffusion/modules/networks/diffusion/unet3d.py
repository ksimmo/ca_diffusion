...
from ca_diffusion.modules.conv.blocks3d import ResnetBlock3D, Attention3D
...
class Encoder(nn.Module):
    def __init__(self, channels_in, channels, channels_z, channel_mult=[2,4], num_blocks=2, attn_res=[], num_heads=8, qkv_bias=True, qk_norm=True,
        ...
        self.proj_in = nn.Conv3d(channels_in, channels, kernel_size=3, stride=1, padding=1)
        ...
        self.proj_out = nn.Sequential(nn.GroupNorm(32, channels_b), nn.SiLU(),
                                      nn.Conv3d(channels_b, channels_z, kernel_size=3, stride=1, padding=1, bias=True))
